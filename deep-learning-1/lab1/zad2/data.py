...
def eval_perf_multi(Y, Y_):
  ...
  #   tp = np.sum((Y  == label) & (Y_ == label))
  #   fp = np.sum((Y_ != label) & (Y  == label))
  #   fn = np.sum((Y  != label) & (Y_ == label))
  #   precisions.append(tp/(tp+fp))
  #   recalls.append(tp/(tp+fn))
  
  return accuracy, pr, M#, np.mean(np.array(precisions)), np.mean(np.array(recalls))
