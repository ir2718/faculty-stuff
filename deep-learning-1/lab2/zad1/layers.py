...
class FC(Layer):
  ...
  def backward_params(self, grads):
    ...
    """
    grad_weights = grads.T @ self.last
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    Returns:
      ndarray of shape (N, C, H, W).
    """
    return (self.last > 0).astype(float) * grads
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    return np.mean(np.log(np.sum(np.exp(x), axis=1)) - np.sum(y*x, axis=1))
  ...
  def backward_inputs(self, x, y):
    ...
    """
    softmax_ = (np.exp(x).T/np.sum(np.exp(x), axis=1)).T
    diff = softmax_ - y
    diff_scaled = diff / x.shape[0]
    return diff_scaled
