...
class FeaturePyramidNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        res5_channel_proj = self.channel_projections[-1](x['res5'])
        res5_blend_conv = self.blend_convs[-1](res5_channel_proj)

        out['fpn_pool'] = F.max_pool2d(res5_blend_conv, kernel_size=1, stride=2)
        out['fpn5'] = res5_blend_conv
        
        res4_channel_proj = self.channel_projections[-2](x['res4'])
        s = res4_channel_proj.shape
        upsampled_res5_channel_proj = F.interpolate(
            res5_channel_proj, 
            size=(s[2], s[3]), 
            mode='nearest'
        )
        sum_res4 = res4_channel_proj + upsampled_res5_channel_proj
        out['fpn4'] = self.blend_convs[-2](sum_res4)


        res3_channel_proj = self.channel_projections[-3](x['res3'])
        s = res3_channel_proj.shape
        upsampled_res4_channel_proj = F.interpolate(
            sum_res4, 
            size=(s[2], s[3]), 
            mode='nearest'
        )
        sum_res3 = res3_channel_proj + upsampled_res4_channel_proj
        out['fpn3'] = self.blend_convs[-3](sum_res3)


        res2_channel_proj = self.channel_projections[-4](x['res2'])
        s = res2_channel_proj.shape
        upsampled_res3_channel_proj = F.interpolate(
            sum_res3, 
            size=(s[2], s[3]), 
            mode='nearest'
        )
        sum_res2 = res2_channel_proj + upsampled_res3_channel_proj
        out['fpn2'] = self.blend_convs[-4](sum_res2)
